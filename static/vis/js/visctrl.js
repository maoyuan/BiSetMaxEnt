...
$(document).ready(function() {
    ...
    visCtrlRequest(rstJSON, "dimGraph");
});
...
function visCtrlRequest(rJson, rType) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            switch (rType) {
                case "loadVisList":
                    loadVisListHelper(repData);
                    break;
                case "addVis":
                    saveVisConfigHelper(repData);
                    break;
                case "saveVis":
                    saveVisHelper(repData);
                    break;
                case "deleteVis":
                    {
...
function loadVisHelper(resData) {
    ...
    for (var i = 0; i < selDims.length; i++) {
        ...
        var aList = canvas.append('g')
            ...
            .attr("transform", function(d, i) {
                return "translate(" + biset.entList.startPos + "," + 0 + ")";
            });
        entLists.push(aList);
    ...
    for (var i = 0; i < selDims.length - 1; i++) {
        ...
        var aBicList = canvas.append('g')
            ...
            .attr("transform", function(d, i) {
                return "translate(" + bicStartPos + "," + 0 + ")";
            });
